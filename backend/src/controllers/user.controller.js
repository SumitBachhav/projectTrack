...
const registerStudent = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    [userID, year, division, department, certificates, github].some(
      (field) => field?.trim() === ""
    )
  ) {
    throw new ApiError(400, "All fields are required");
  ...
  await User.updateOne(
    { _id: req.user._id },
    { $set: { childId: createdStudent._id } },
    { new: true }
  ).catch((error) => {
    throw new ApiError(
      500,
...
const registerStaff = asyncHandler(async (req, res) => {
  ...
  if (!createdStaff) {
    throw new ApiError(
      500,
      `Something went wrong while registering the staff - ${error.message}`
    );
  }

  await User.updateOne(
    { _id: req.user._id },
    { $set: { childId: createdStaff._id } },
    { new: true }
  ).catch((error) => {
    throw new ApiError(
      500,
...
const getAllUsers = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { names }, "User names fetched successfully"));
  } catch (error) {
