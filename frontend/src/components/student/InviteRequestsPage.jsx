...
const InviteRequestsPage = () => {
  ...
  const [modifiableInviteId, setModifiableInviteId] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAcceptedInviteId(
          response.data.data.receivedInvites.find(
          ...
          <div key={key} className="mb-6">
            ...
            {list.length === 0 ? (
              <p className="text-gray-500 mt-2">
                No {key.replace(/([A-Z])/g, " $1").toLowerCase()}.
              </p>
            ) : (
              list.map((item, index) => (
                <div
                  key={index}
                  className={`mt-3 p-4 border rounded-lg shadow-sm bg-gray-50 ${
                    acceptedInviteId && acceptedInviteId !== item._id
                      ? "opacity-50"
                      : ""
                  }`}
                >
                  ...
                  {key === "receivedInvites" && (
                    <div className="mt-3 flex space-x-3">
                      {modifiableInviteId === item._id ||
                      (item.status === "pending" && !acceptedInviteId) ? (
                        <>
                          <button
                            className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600"
                            onClick={() => handleResponse(item.inviteId, "accepted")}
                          >
                            Accept
                          </button>
                          <button
                            className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600"
                            onClick={() => handleResponse(item.inviteId, "rejected")}
                          >
                            Reject
                          </button>
                          <button
